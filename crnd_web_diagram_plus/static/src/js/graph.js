...
function Connector (graph, node, pos_x, pos_y) {
        ...
        var drag_down = function () {
            ...
            // eslint-disable-next-line no-use-before-define
            self.edge_tmp = new GraphEdge(
                graph, '', self.edge_start, self.edge_end, true);
        ...
        var drag_up = function () {
            ...
            if (graph.target_node) {
                // eslint-disable-next-line no-use-before-define
                var edge_prop = GraphEdge.creation_callback(
                    node, graph.target_node);
                if (edge_prop) {
                    // eslint-disable-next-line no-use-before-define
                    var new_edge = new GraphEdge(
                        graph, edge_prop.label, node, graph.target_node);
                    // eslint-disable-next-line no-use-before-define
...
function Graph (r, style, viewport) {
        ...
        var edges = [];
        // The graph[n1.uid][n2.uid] -> list of all edges from n1 to n2
        var graph = {};
        // The links[n.uid] -> list of all edges from or to n
        var links = {};
        // All nodes and edges have an uid used to order their display when
        // they are curved
        var uid = 1;
        ...
        // This holds the target node when creating an edge and hovering a
        // connector
        self.target_node = null;
        // The raphael instance
        self.r = r;
        // Definition of the colors, spacing, fonts, ... used by the elements
        self.style = style;
        // Global translation coordinate
        var tr_x = 0, tr_y = 0;

        var background = r.rect(0, 0, '100%', '100%').attr(
            {'fill': 'white', 'stroke': 'none', 'opacity': 0, 'cursor': 'move'}
        );
        ...
        // Elements inserted in the graph after a translate_all() must manually
        // apply transformation via get_transform()
        ...
        var translation_respects_viewport = function (dx, dy, margin) {
            ...
            var margin_norm = margin || 0;
            var b = get_bounds();
            var width = viewport.offsetWidth;
            var height = viewport.offsetHeight;

            if (dy < 0 && b.maxy + dy < margin_norm ||
                dy > 0 && b.miny + dy > height - margin_norm ||
                dx < 0 && b.maxx + dx < margin_norm ||
                dx > 0 && b.minx + dx > width - margin_norm) {
                return false;
            }

            return true;
        };
        // Adds a mousewheel event callback to raph_element that scrolls the
        // viewport
        this.set_scrolling = function (raph_element) {
            $(raph_element.node).bind('mousewheel', function (event, delta) {
                var dy = delta * 20;
                if (
                    translation_respects_viewport(0, dy, style.viewport_margin)
                ) {
                    translate_all(0, dy);
                }
            });
        ...
        // Return a curvature index so that all edges connecting n1, n2 have
        ...
        // Returns the angle in degrees of the edge loop.
        // We do not support more than 8 loops on one node
        this.get_loop_angle = function (n, e) {

            // The 8 angles where we can put the loops
            var slots = [];
            for (var angle = 0; angle < 360; angle += 45) {
                slots.push(Vec2.new_polar_deg(1, angle));
            }

            // We assign to each slot a score. The higher the score,
            // the closer it is to other edges.
            var links_ = this.get_linked_edge_list(n);
            ...
            // We want the loops with lower uid to get
            // the slots with the lower score
            slots.sort(function (a, b) {
                return a.score < b.score ? -1: 1;
            });

            var index = 0;
            for (var j = 0; j < links_.length; j++) {
                edge = links_[j];
                if (!edge.is_loop || !edge.is_loop()) {
                    continue;
                }
...
function GraphNode (graph, pos_x, pos_y, label, type, color, color_label) {
        ...
        var get_pos = function () {
            ...
            return new Vec2(
                node_fig.attr('x') + sx / 2,
                node_fig.attr('y') + sy / 2
            );
        };
        ...
        var set_selected = function () {
            if (!selected) {
                ...
                if (!self.close_button) {
                    self.close_button = new CloseButton(
                        graph, self, "node", sx / 2, - sy / 2);
...
function GraphEdge (graph, label, start, end, tmp) {
        ...
        // eslint-disable-next-line init-declarations
        var s, e;
        // Position of the middle of the curve (bezier control point)
        // eslint-disable-next-line init-declarations
        var mc;
        // Control points of the cubic bezier for the loop edges
        // eslint-disable-next-line init-declarations
        var mc1, mc2;
        var elfs = graph.style.edge_label_font_size || 10;
        var label_enabled = true;
        // Unique id used to order the curved edges
        this.uid = 0;
        // The svg definition of the edge vector path
        var edge_path = "";
        var selected = false;
        ...
        function get_label_pos (path) {
            ...
            var verticality = Math.abs(
                end.get_pos().sub(start.get_pos()).normalize().dot_xy(0, 1));
        ...
        this.get_pos = function () {
            ...
            // The var bbox = edge_label.getBBox(); Does not work... :(
        ...
        function update_curve () {
            ...
            if (start !== end) {
                ...
                if (start.get_bound) {
                    col = start.get_bound().collide_segment(s, mc);
                    if (col.length > 0) {
                        s = col[0];
                    }
                }
                if (end.get_bound) {
                    col = end.get_bound().collide_segment(mc, e);
                    if (col.length > 0) {
                        e = col[0];
                    }
                }
                ...
                var r_ = Vec2.new_polar_deg(
                    rad, graph.get_loop_angle(start, self));
                mc = s.add(r_);
                var p = r_.rotate_deg(90);
                mc1 = mc.add(p.set_len(rad * 0.5));
                mc2 = mc.add(p.set_len(- rad * 0.5));

                if (start.get_bound) {
                    col = start.get_bound().collide_segment(s, mc1);
                    if (col.length > 0) {
                        s = col[0];
                    }
                    col = start.get_bound().collide_segment(e, mc2);
                    if (col.length > 0) {
                        e = col[0];
                    }
                }
        ...
        // Since we create an edge we need to recompute the edges that have the
        // same start and end positions as this one
        if (!tmp) {
            var edges_start = graph.get_linked_edge_list(start);
            var edges_end = graph.get_linked_edge_list(end);
            var edges = edges_start.length < edges_end.length
        ...
        function update () {
            ...
            if (label_enabled) {
                var labelpos_ = get_label_pos(edge);
                edge_label.attr({'x': labelpos_.x, 'y': labelpos_.y - 14});
            }
        }
